...
const { maxNumber } = await SELECT.one`max(reqno) as maxNumber`.from(RequestHeaders);
  ...
  srv.on('responsefrombpa',async(req)=>{
    ...
    const payload_bpa_header = await SELECT.from(RequestHeaders).where({ reqno: req.data.ID });
    const payload_bpa_items = await SELECT.from(RequestItems).where({ RequestHeaders_ID: req.data.ID });
